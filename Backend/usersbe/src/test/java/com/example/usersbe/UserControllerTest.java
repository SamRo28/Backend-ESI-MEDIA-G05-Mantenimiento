...
class UserControllerTest {
        ...
        void setup() {
                ...
                // Clean up rate limit log file to prevent 429 Too Many Requests
                java.io.File logFile = new java.io.File(System.getProperty("java.io.tmpdir"), "forgot-password.log");
                if (logFile.exists()) {
                        logFile.delete();
                }
        }
        ...
        void updateCreador_y_updateUser_ok() throws Exception {
                ...
                when(userService.updateCreadorContenido(anyString(), any(), any(), any(), any(), any(), any(), any()))
                                .thenReturn(u);
                // Corrected mock: 7 arguments (including misGustos)
                when(userService.updateProfile(anyString(), any(), any(), any(), any(), any(), any()))
                                .thenReturn(u);
        ...
        void admin_users_endpoints_ok() throws Exception {
                ...
                doThrow(new com.example.usersbe.exceptions.UserDeletionNotAllowedException(
